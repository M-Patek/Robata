...
class PipelineFixture:
    ...
    inference_requests: tuple[VisionInferenceRequest, ...]
    inference_outcomes: tuple[VisionInferenceOutcome, ...]
    qa_aggregates: tuple[QAResultAggregate, ...]
...
def test_package_requires_exact_six_camera_slots(mutation: str) -> None:
    package = _pipeline_fixture().packages[0]
...
def test_nanoseconds_use_canonical_json_strings_and_half_open_intervals() -> None:
    ...
    window_payload = _pipeline_fixture().windows[0].model_dump(mode="json")
...
def test_provider_claims_reject_authoritative_persisted_ids(
    ...
    output = _pipeline_fixture().inference_outcomes[outcome_index].output
