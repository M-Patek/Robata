...
class QAMatrixReport:
    ...
    def failures(self) -> tuple[QAMatrixCase, ...]:
        return tuple(
            ...
            if case.local_status is QAStatus.PASS
            or case.full_coverage_status is QAStatus.PASS
        )
...
def validate_issue_matrix(*, duration_sec: float = 20.0) -> QAMatrixReport:
    ...
    for ordinal, issue in enumerate(QAIssue):
        ...
        if local.status is not expected_local or full.status is not expected_full:
            # Preserve observed values for diagnostics; mark report as failed below.
            pass
        cases.append(
            QAMatrixCase(
                issue=issue,
    ...
    report = QAMatrixReport(issue_count=len(cases), cases=tuple(cases), passed=False)
    expected = {
        issue: (
            QAStatus.FAIL
            if disposition is IssueDisposition.WHOLE_RECORDING_FAIL
