...
|---|---|---|
| Contract and schema kernel | Strict values, canonical JSON/SHA-256, six-camera maps, atomic single-schema and evolution-bundle registration, immutability checks, logical nodes, immutable revisions, current-selection primitives, and 44 exact-pinned schemas are executable. An evolution bundle publishes one new target, one or more incoming upcasters, and their exact code/runtime/golden artifacts only after complete temporary registry and graph validation. Registered payloads include compact/detailed completion, output, work-message, persisted-barrier, event-identity outbox, review-task, review-annotation, and review-reopen-command contracts. Compact V1 bytes remain frozen and exact-readable without an implicit upcast. | The live catalog still declares `upcasters=[]`. The evolution command proves publication mechanics only. Existing completion targets were already published with `compatibility_mode=NONE` and no predecessors, so they cannot be retrofitted without violating catalog immutability; detailed V1 also lacks facts required by later versions. A future governed chain must target new versions. Detailed-result bytes remain embedded in local SQLite rather than a production artifact store. |
...
| Queueing | `queue/barrier.py` provides deterministic aggregation and `adapters/sqlite_barrier.py` persists generic/inference-call barriers plus an exact-pinned persisted-barrier snapshot. `SQLiteWorkScheduler` provides an authoritative durable ledger with dependency progression, atomic claim/CAS, lease epoch and fencing token, retry wait, deadlines, hard expiry, cancellation, skip, invalidation, and restart recovery. `OutboxRelay` plus SQLite delivery state exact-validates the registered event-identity outbox Wire payload and provides local at-least-once delivery, deterministic retry, idempotent sink behavior, acknowledgement, and terminal DLQ simulation. | These local SQLite components are not yet wired into the canonical composition as one production topology. Redis/broker selection, production concurrency/isolation, reconciliation ownership, metrics, and O-14 recovery policy remain open. |
...
| 25.2 provider-neutral packages and durable call barrier | PARTIAL | Temporal packages remain provider-neutral; provider limits and render details live in `InferenceInputPlan`; independently retried call parts and deterministic reduction are executable. The local composition persists generic and inference-call barrier facts and can recover without provider redispatch. The persisted generic-barrier snapshot and lease-bound work-message projection now have registered exact schemas; a separate SQLite scheduler proves lease/fence recovery. | The scheduler is not yet the canonical composition authority, and Redis/broker production topology, real provider qualification, production isolation, and O-14 recovery ownership remain open. |
| 25.3 exact and adaptive sampling | PARTIAL | Exact rational-grid selection and materialization are tested. `sampling/adaptive.py` deterministically resolves a frozen trigger artifact into reduced rational segments or explicit integer-nanosecond targets. Independent Python and Node BigInt implementations verify the checked-in half-even, negative-index, clipping, tolerance, tie-break, decode-failure, and dedupe vectors. | `AdaptiveSampler.sample()` is fail-closed. Signal detection and trigger-to-rate policy remain absent; O-13 owns promoted rates, padding, trigger policy, and budgets. |
...
| 25.7 exact schema references and evolution | PARTIAL | Atomic single-schema registration and atomic publication of one new evolution target with 1..N incoming upcasters are executable. The bundle stages exact schema/code/runtime/golden artifacts, validates the complete temporary registry and upcaster graph, and makes the catalog replacement the commit point with marker-based recovery. Exact digest verification, published-entry immutability, deterministic upcasting, and golden-vector enforcement are tested. Standalone output decision, admission proof, event hypothesis, work message, persisted barrier, event-identity outbox, review task, review annotation, and review reopen command payloads have exact-pinned schemas and validators in the 44-entry catalog. | The live catalog still has no domain upcaster (`upcasters=[]`); bundle publication is mechanism evidence, not a published business migration. Existing completion versions were published with `NONE` compatibility and empty predecessor sets, so retroactive V1->V2->V3/V4 edges would mutate published governance metadata; detailed V1 also lacks later required facts. New governed target versions are required. |
| 25.8 nonblocking human review | PARTIAL | Versioned nonblocking routing covers the five Section 25.8 triggers. Immutable review tasks, annotations/adjudications, optional revision/selection references, priority/SLA ordering, lease/fence, expired reclaim, exact replay/conflict, reopen history, and independently visible pending/overdue state are executable in a local SQLite queue. Task, annotation, and reopen-command payloads are registered contracts and the queue validates their exact pins on write and read. Queue failure cannot replace or withhold the primary result; ungoverned blocking review fails closed. | Review routing is not wired into the canonical composition. Governed O-10/O-11/O-12 routing and blocking policies, reviewers, capacity, backlog/latency evidence, and operational SLA ownership remain absent. |
| 25.9 recording-scoped serialized identity | PARTIAL | The one-command local path obtains a recording snapshot, prepares identity side-effect-free, and applies it with run completion, ActionEvent genesis, and outbox under one generation/fence CAS transaction. `EventIdentityOutboxRecord` retains its original domain shape where it is embedded in frozen completion detail; SQLite persistence and publication project it to the exact-pinned `EventIdentityOutboxWireRecord`. Exact replay creates no duplicate business result or outbox. A separate SQLite relay proves local at-least-once delivery, retry, acknowledgement, idempotent sink behavior, stale-fence rejection, and DLQ simulation. | The aggregate and relay are local conformance evidence, not the O-14 production authority. Canonical relay composition, production broker delivery/reconciliation, and governed merge/split/REUSED successor behavior remain absent. |
...
| 25.11 promotion evidence | PARTIAL | Focused local tests cover canonical 0/1/N execution, replay, boundary refinement, call reduction, SQLite inference/barrier/work/outbox/review recovery, atomic completion, exact Wire validation, Python/Node rational-grid vectors, content-addressed benchmark policy, and synthetic capacity/SLO regression semantics. | Governed source/provider replay, representative production load, production outbox/broker evidence, approved metric/review policies, frozen ground truth, thresholds, security regression, and long soak remain missing. |
...
| `queue/barrier.py` | PARTIAL | The coordinator and terminal criticality semantics are executable. SQLite persists generic and inference-call facts, emits a registered exact-pinned barrier snapshot, and proves replay/reopen/crash recovery. A separate durable scheduler supplies deadline/lease/fence/invalidation behavior and emits a registered lease-bound work-message projection. Production broker composition, concurrency qualification, and O-14 recovery policy remain external. |
...
- Section 25.7 governed domain upcaster selection. Atomic publication infrastructure now accepts
  ...
  the complete temporary registry and graph before the catalog commit. The 44-entry catalog
  includes work-message,
  persisted-barrier, event-identity outbox, review-task, review-annotation, and review-reopen-command
  contracts beside the standalone V2 output contracts. It still declares `upcasters=[]`: existing
...
- Production barrier/work recovery. The local SQLite scheduler now proves dependency progression,
  deadlines, leases/fences, retry, invalidation, and restart recovery; the canonical runner still
  uses its existing run-scoped barrier/evidence composition. Broker delivery, scheduler wiring,
  production concurrency/isolation, and O-14 recovery ownership remain absent.
- Production outbox delivery. A local SQLite relay now proves at-least-once retry, acknowledgement,
  stale-fence rejection, idempotent delivery, and DLQ simulation. It is not composed into the
  canonical command and does not select a production broker or reconciliation authority.
...
mapped raw MCAP, with processing-run membership, identity preparation, ActionEvent genesis, atomic
completion, and pending outbox. Same-run and cross-run replay reuse durable logical/inference facts
without provider redispatch. Separate local SQLite components now prove durable work scheduling,
lease/fence recovery, registered work/barrier projections, and outbox relay/DLQ behavior. They are
not yet composed as the production execution topology: external artifact storage, broker choice,
production isolation/reconciliation, governed provider routes, and O-14 recovery ownership remain
open. Work behind an authority gate still stops at an explicit port, policy input, or fail-closed
state.

Nonblocking review routing, a local priority/SLA review queue, Python/Node rational-grid vectors,
and a synthetic capacity/SLO harness are executable internal evidence. The review queue persists
exact-pinned task, annotation, and reopen-command contracts. None supplies governed review policy,
real reviewer capacity, representative provider load, or production qualification. The RunPod
