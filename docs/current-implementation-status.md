...
|---|---|---|
| Contract and schema kernel | Strict values, canonical JSON/SHA-256, six-camera maps, atomic single-schema and evolution-bundle registration, immutability checks, logical nodes, immutable revisions, current-selection primitives, and 47 exact-pinned schemas are executable. An evolution bundle publishes one new target, one or more incoming upcasters, and their exact code/runtime/golden artifacts only after complete temporary registry and graph validation. Registered payloads include compact/detailed completion, output, work-message, persisted-barrier, event-identity outbox, review-task, review-annotation, review-reopen-command, media-quality-report, and supplemental-QA-evidence contracts. Compact V1 bytes remain frozen and exact-readable without an implicit upcast. | The live catalog still declares `upcasters=[]`. The evolution command proves publication mechanics only. Existing completion targets were already published with `compatibility_mode=NONE` and no predecessors, so they cannot be retrofitted without violating catalog immutability; detailed V1 also lacks facts required by later versions. A future governed chain must target new versions. Detailed-result bytes remain embedded in local SQLite rather than a production artifact store. |
...
- Schema registry verification passed for 47 exact-pinned JSON Schema 2020-12 documents and zero
